...
void ft_signal() 
{
    //sa maini meja arvum, aysinq minishelli himnakan processum, voch ayl processneri, aly looperi mej(heredoc), cat ayl petq e aytl zev asxaten

    handle_term(); //

    signal(SIGINT, sigint_handler); //ctrl-C, nor tox e ancnum, tpum prompty noric u jnjum ^C
   signal(SIGQUIT, SIG_IGN);

    //signal(SIGQUIT); //ignore ctrl |
   // signal()
...
void ft_loop(t_token_list *token_list, t_environment_list *envp_list) 
{
    ...
    while (1) 
    {
        ...
        if (input_str == NULL) 
        {
            printf("exit"); //exit statusy 1 poxel, ctrl-d, eof, 
